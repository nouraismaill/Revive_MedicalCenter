import FormatDate from "../../utils/formatDate";
const DoctorAbout = () => {
  return (
    <div>
      <div>
        <h3 className="text-[20px] leading-[30px] text-black font-semibold flex items-center gap-2">
          About of
          <span className="text-blue-500 font-bold text-[24px] leading-9">
            Muhib Rafi
          </span>
        </h3>
        <p className="text__para">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Obcaecati
          debitis, ipsum enim accusamus voluptates, itaque doloribus quasi
          explicabo laborum, id exercitationem eaque iste ipsam inventore
          facere. Suscipit dolores molestiae, necessitatibus amet fugit vero
          accusantium!
        </p>
      </div>
      <div className="mt-12">
        <h3 className="text-[20px] leading-[30px] text-black font-semibold ">
          Education
        </h3>
        <ul className="pt-4 md:p-5">
          <li className="flex flex-col sm:flex-row sm:justify-between sm:items-end md:gap-5 mb-[30px]">
            <div>
              <span className="text-blue-500 text-[15px] leading-6 font-semibold">
                {FormatDate("09-13-2014")} - {FormatDate("08-08-2016")}
              </span>
              <p className="text-[16px] leading-6 font-medium text-gray-400">
                PHD in Surgeon
              </p>
            </div>
            <p className="text-[14px] leading-5 font-medium text-gray-400">
              New Apip Hospital,New York
            </p>
          </li>
          <li className="flex flex-col sm:flex-row sm:justify-between sm:items-end md:gap-5 mb-[30px]">
            <div>
              <span className="text-blue-500 text-[15px] leading-6 font-semibold">
                {FormatDate("07-04-2010")} - {FormatDate("08-08-2014")}
              </span>
              <p className="text-[16px] leading-6 font-medium text-gray-400">
                PHD in Surgeon
              </p>
            </div>
            <p className="text-[14px] leading-5 font-medium text-gray-400">
              New Apip Hospital,New York
            </p>
          </li>
        </ul>
      </div>
      <div className="mt-12">
        <h3 className="text-[20px] leading-[30px] text-black font-semibold ">
          Experience
        </h3>
        <ul className="grid sm:grid-cols-2 gap-[30px] pt-4 md:p-5 ">
          <li className="p-4 rounded bg-[#fff9ea]">
            <span className="text-blue-500 text-[15px] leading-6 font-semibold">
              {FormatDate("07-04-2010")} - {FormatDate("08-08-2014")}
            </span>
            <p className="text-[16px] leading-6 font-medium text-gray-400">
              Sr. Surgeon
            </p>
            <p className="text-[14px] leading-5 font-medium text-gray-400">
              New Apip Hospital,New York
            </p>
          </li>
          <li className="p-4 rounded bg-[#fff9ea]">
            <span className="text-blue-500 text-[15px] leading-6 font-semibold">
              {FormatDate("07-04-2010")} - {FormatDate("08-08-2014")}
            </span>
            <p className="text-[16px] leading-6 font-medium text-gray-400">
              Sr. Surgeon
            </p>
            <p className="text-[14px] leading-5 font-medium text-gray-400">
              New Apip Hospital,New York
            </p>
          </li>
        </ul>
      </div>
    </div>
  );
};

export default DoctorAbout;
